...
import kamau_technerd.com.weekenders.adapters.wholeCustomAdapter;
import kamau_technerd.com.weekenders.adapters.wholeList;
...
public class whole extends AppCompatActivity {
    ...
    List<wholeList> list = new ArrayList<>();
            ...
            ValueEventListener mDBListener = refb.addValueEventListener(new ValueEventListener() {
                ...
                public void onDataChange(DataSnapshot dataSnapshot) {
                    ...
                    for (DataSnapshot postSnapshot : dataSnapshot.getChildren()) {
                        ...
                        if (i % 2 == 0)
                            list.add(new wholeList(imageid1, imageid2));
                        i++;
                    }
                    if (i % 2 == 0)
                        list.add(new wholeList(imageid1, "0"));
                    ...
                    public void onDataChange(DataSnapshot dataSnapshot) {
                        for (DataSnapshot postSnapshot : dataSnapshot.getChildren()) {
                            if (postSnapshot.getKey().toLowerCase().contains(stext.toLowerCase())) {
                                ...
                                if (k[0] % 2 == 0)
                                    list.add(new wholeList(imageid[0], imageid[1]));
                                k[0]++;
                            }
                        }
                        if (k[0] % 2 == 0) {
                            list.add(new wholeList(imageid[0], "0"));
                            k[0]++;
                        }
                        myAdapter.notifyDataSetChanged();
                        mProgressCircle.setVisibility(View.INVISIBLE);
